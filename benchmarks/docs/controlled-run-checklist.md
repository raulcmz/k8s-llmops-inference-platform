...
A **controlled run** means: change one thing at a time, write down the setup, and only then trust the numbers.

Use this every time before pasting results into notes or a README.
...
- [ ] Nobody else is hammering the same model (shared host = noisy numbers)
...
- [ ] Save the command that was run (copy/paste into the template)
...
- [ ] Optional: compare with gateway Prometheus:

```bash
curl -s http://127.0.0.1:8080/metrics | rg 'llm_ttft|llm_tpot|llm_errors|llm_request_duration'
```
